...
function Start-WinDocumentationServer {
    ...
    foreach ($Domain in $ForestInformation.Domains) {
        ...
        ### Section - Group Policies
        $SectionPasswordPolicies = $WordDocument | Add-WordTocItem -Text 'General Information - Priviliged Members' -ListLevel 2 -ListItemType Numbered -HeadingType Heading2
        $SectionPasswordPolicies = $WordDocument | Get-WordDomainPriviligeMembers -Paragraph $SectionPasswordPolicies -ActiveDirectorySnapshot $ActiveDirectorySnapshot -Domain $Domain #-Verbose
    }
